/**
 * Vehicle Search & Filter Implementation
 */

document.addEventListener('DOMContentLoaded', function() {
    const searchInput = document.getElementById('global-search');
    if (searchInput) {
        searchInput.placeholder = "Search license plate, brand, or model...";
    }
    ...
    function filterTable() {
        ...
        tableRows.forEach(row => {
            const licensePlate = row.querySelector('td:nth-child(1)').textContent.toLowerCase();
            const modelInfo = row.querySelector('td:nth-child(2)').textContent.toLowerCase();
            const typeInfo = row.querySelector('td:nth-child(3)').textContent.toLowerCase();
            ...
            const matchesSearch = licensePlate.includes(searchTerm) || 
                                modelInfo.includes(searchTerm) || 
                                typeInfo.includes(searchTerm);
        ...
        if (visibleRows.length === 0) {
            if (!emptyMessage) {
                ...
                tr.id = 'noVehiclesMessage';
                tr.innerHTML = `<td colspan="6" style="padding: 4rem; text-align: center; color: var(--text-dim);">No vehicles found matching your search.</td>`;
